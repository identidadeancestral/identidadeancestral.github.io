...
export default function OjiisanApp(access:AccessProps) {
 const [lang,setLang]=useState<Lang>("pt"),[view,setView]=useState("learn"),[lessonId,setLessonId]=useState("method:location"),[draft,setDraft]=useState<BlocksPayload>(()=>defaultBlocks());
 ...
 const [voices,setVoices]=useState<SpeechSynthesisVoice[]>([]),[speaking,setSpeaking]=useState(false),[chatVisited,setChatVisited]=useState(false),[seed,setSeed]=useState<{payload:MessagePayload;id:number}>(),[profileRequest,setProfileRequest]=useState(0),[example,setExample]=useState(false);
