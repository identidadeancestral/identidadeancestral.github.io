...
import { methodPatterns,patternById,defaultBlocks,methodChoices,methodNoun,methodPlaces,methodForms,hasForm,hasQuestion,composeBlocks,methodAdverbChoices,type BlocksPayload } from "@/lib/method-blocks";
...
export default function BlockComposer({value,onChange,lang,showPattern=true,preview=true,onSend,busy=false}:{value:BlocksPayload;onChange:(p:BlocksPayload)=>void;lang:Lang;showPattern?:boolean;preview?:boolean;onSend?:()=>void;busy?:boolean}) {
 ...
 const patch=(p:Partial<BlocksPayload>)=>{const next={...value,...p};if(next.adverb&&!methodAdverbChoices(next).includes(next.adverb))delete next.adverb;onChange(next);};
 ...
 return <div className="block-composer">
  <div className="block-fields">
   ...
   {!!methodAdverbChoices(value).length&&select(t("Advérbio opcional","Optional adverb"),value.adverb||"none",[{id:"none",label:t("Sem advérbio","No adverb")},...methodAdverbChoices(value).map(v=>({id:v,label:entryById["adverb:"+v].jp+" · "+entryById["adverb:"+v][lang]}))],adverb=>patch({adverb:adverb==="none"?undefined:adverb}))}
