...
type Mode="blocks"|"reading"|"japanese";
...
const errors:Record<string,[string,string]>={
  ...
  gateway_auth:["Falha no acesso ao servidor. Sua sessão foi mantida; tente novamente em instantes.","Server access failed. Your session was kept; please try again shortly."],
  ...
  invalid_message:["Confira as peças do molde antes de enviar.","Check the pattern pieces before sending."],
};
...
function Tokens({payload,mode,lang,onWord}:{payload:MessagePayload;mode:Mode;lang:Lang;onWord:(word:Word)=>void}) {
 return <MessageBlocks payload={payload} lang={lang} plain={mode==="japanese"} reading={mode==="reading"} onWord={onWord}/>;
}
...
export default function ChatApp({signedIn,signInPath,signOutPath,apiOrigin,sessionToken,onSignOut,onSignIn,onManageAccount,langOverride,composeSeed,profileRequest}:AccessProps & {langOverride?:Lang;profileRequest?:number;composeSeed?:{payload:MessagePayload;id:number}}) {
  ...
  const [composeMode,setComposeMode]=useState("blocks");
  const [blockDraft,setBlockDraft]=useState<BlocksPayload>(()=>defaultBlocks());
  ...
  const draft:MessagePayload=composeMode==="blocks"?blockDraft:composeMode==="text"?{kind:"text",text}:composeMode==="story"?{kind:"story",story:storyId}:{kind:"visual",template,noun,question};
  useEffect(()=>{if(composeSeed){const p=composeSeed.payload;if(p.kind==="blocks"){setBlockDraft(p);setComposeMode("blocks");}else if(p.kind==="story"){setStoryId(p.story);setComposeMode("story");}else{setText(compose(p).japanese);setComposeMode("text");}setMobileChat(true);}},[composeSeed]);
  ...
  useEffect(()=>{try{const l=localStorage.getItem("kotoba-language");if(langOverride)setLang(langOverride);else if(l==="en"||l==="pt")setLang(l);else if(!navigator.language.startsWith("pt"))setLang("en");const m=localStorage.getItem("kotoba-mode");if(["blocks","reading","japanese"].includes(m||""))setMode(m as Mode);}catch{}},[langOverride]);
  ...
  const demo:MessagePayload=defaultBlocks();
  ...
  return <div className="app-frame">
    ...
    <div className={"chat-shell"+(mobileChat?" mobile-chat":"")}>
      <aside className="people-panel">
        ...
        <div className="learning-shortcuts"><Button variant="outline" onClick={()=>{setStudyEntry("");setLibraryOpen(true);}}><BookOpen/>{t("400 palavras e flexões","400 words and forms")}</Button><a href={EBOOK_URL} target="_blank" rel="noopener noreferrer">{t("Conhecer o ebook · Método 100 Blocos","Explore the ebook · Método 100 Blocos")}<ArrowUpRight/></a></div>
        {!signedIn?<div className="join-panel"><span className="eyebrow">{t("UMA CONVERSA DE CADA VEZ","ONE CONVERSATION AT A TIME")}</span><h1>{t("Seu japonês ganha companhia.","Your Japanese finds company.")}</h1><p>{t("Encontre pessoas, forme grupos e converse usando blocos em japonês.","Meet people, make groups and chat using Japanese sentence blocks.")}</p><Button size="lg" className="full-button" onClick={onSignIn}>{t("Entrar com e-mail","Sign in with email")}<ArrowUpRight/></Button><p className="small-note">{t("Na primeira entrada, você cria seu perfil. Seu e-mail não aparece para as outras pessoas.","On your first visit, create your profile. Your email is not shown to other people.")}</p><Button variant="outline" onClick={practiceRoom}>{t("Experimentar os blocos","Try the blocks")}<ChevronRight/></Button></div>
      ...
      <main className="conversation-panel">
        ...
        <div className="reading-bar"><span><BookOpen/>{t("Como você lê","How you read")}</span><Choice label={t("Modo de leitura","Reading mode")} value={mode} onChange={v=>setMode(v as Mode)} items={[{value:"blocks",label:t("Blocos por função","Blocks by role")},{value:"reading",label:t("Blocos + leitura","Blocks + reading")},{value:"japanese",label:t("Só japonês","Japanese only")}]}/></div>
        ...
        <div className="message-stream" role="log" aria-label={t("Mensagens","Messages")} aria-live="polite">
          {!active&&!rendered.length&&<div className="practice-intro"><h2>{t("Monte uma ideia e envie os blocos.","Build an idea and send the blocks.")}</h2><p>{t("Escolha um molde abaixo. Depois de enviar, veja como cada parte funciona.","Choose a pattern below. After sending, see how each part works.")}</p><BlockLegend lang={lang}/><div className="example-bubble"><span className="bubble-label">{t("BLOCO 07 · EXEMPLO","BLOCK 07 · EXAMPLE")}</span><Tokens payload={demo} mode={mode} lang={lang} onWord={showWord}/><MessageExplanation payload={demo} lang={lang}/></div></div>}

          {active&&roomLoading?<div className="message-loading"><Skeleton className="h-24 w-64"/><Skeleton className="ml-auto h-20 w-52"/></div>:active&&!rendered.length?<Empty><EmptyHeader><EmptyTitle>{dmWaiting?t("Convite enviado.","Invitation sent."):t("Sua primeira palavra pode ser um olá.","Your first word could be hello.")}</EmptyTitle><EmptyDescription>{dmWaiting?t("Aguarde a pessoa aceitar para começar a conversa.","Wait for the person to accept before chatting."):t("Escolha um molde ou use uma expressão rápida.","Choose a pattern or use a quick expression.")}</EmptyDescription></EmptyHeader></Empty>:null}
          ...
          {rendered.map((m,index)=><article key={m.id} className={"message-row "+(m.user_id===me?.id||!active?"own":"other")}><div className="message-bubble"><div className="message-author">{m.user_id===me?.id||!active?t("Você","You"):m.nickname}</div>{m.payload.kind==="word"&&<span className="study-message-label">{t("PALAVRA DE ESTUDO","STUDY WORD")}</span>}<Tokens payload={m.payload} mode={mode} lang={lang} onWord={showWord}/><MessageExplanation payload={m.payload} lang={lang} initialOpen={index===rendered.length-1&&(m.user_id===me?.id||!active)}/>{active&&m.user_id!==me?.id&&index===rendered.length-1&&suggestedReplies(m.payload).length>0&&<div className="reply-options"><span>{t("Monte sua resposta","Build your reply")}</span>{suggestedReplies(m.payload).map((p,i)=><Button variant="outline" size="sm" key={i} onClick={()=>{setBlockDraft(p);setComposeMode("blocks");}}>{compose(p).japanese}</Button>)}</div>}<time className="message-time">{new Date(m.created_at).toLocaleTimeString(lang==="pt"?"pt-BR":"en",{hour:"2-digit",minute:"2-digit"})}{active&&m.user_id===me?.id&&<Check aria-label={t("Enviado","Sent")}/>}</time></div></article>)}

          <div ref={bottom}/>
        </div>
        <div className="composer">
          <div className="quick-phrases" aria-label={t("Expressões rápidas","Quick expressions")}>{["hello","thanks","yes","no","again"].map(id=><button key={id} disabled={busy||!!dmWaiting||roomLoading} onClick={()=>send({kind:"visual",template:id})}><span lang="ja">{lexicon[id].jp}</span><small>{lexicon[id][lang].split(" / ")[0]}</small></button>)}</div>
          <Tabs value={composeMode==="text"?"text":"blocks"} onValueChange={setComposeMode}><div className="composer-top"><TabsList><TabsTrigger value="blocks"><BookOpen/>{t("Montar em blocos","Build with blocks")}</TabsTrigger><TabsTrigger value="text"><Languages/>{t("Texto livre","Free text")}</TabsTrigger></TabsList><Button variant="ghost" size="sm" className="open-vocabulary" onClick={()=>{setStudyEntry("");setLibraryOpen(true);}} aria-label={t("Abrir as 400 palavras","Open the 400 words")}><BookOpen/><span>400</span></Button></div>
            <TabsContent value="blocks">
             {composeMode==="story"?<div className="story-composer"><Choice label={t("Histórias em blocos","Stories in blocks")} value={storyId} onChange={setStoryId} items={stories.map(story=>({value:story.id,label:story[lang]}))}/><div className="send-row"><Tokens payload={{kind:"story",story:storyId}} mode={mode} lang={lang} onWord={showWord}/><Button size="icon" className="send-button" disabled={busy||!!dmWaiting||roomLoading} onClick={()=>send()} aria-label={t("Enviar história","Send story")}><Send/></Button></div></div>:composeMode==="visual"?<><div className="sentence-controls"><Choice label={t("Estrutura da frase","Sentence pattern")} value={template} onChange={value=>{setTemplate(value);setNoun(templates.find(x=>x.id===value)!.choices[0]);}} items={templates.map(v=>({value:v.id,label:v[lang]}))}/><div className="question-toggle"><Label htmlFor="question">{t("Pergunta","Question")}</Label><Switch id="question" checked={question||template==="where"} disabled={template==="where"} onCheckedChange={setQuestion}/></div></div><div className="word-keyboard" role="group" aria-label={t("Escolha uma palavra","Choose a word")}>{chosen.choices.map(id=><button key={id} className={id===noun?"picked":""} onClick={()=>setNoun(id)} aria-pressed={id===noun}><span lang="ja">{lexicon[id].jp}</span><small>{lexicon[id][lang]}</small></button>)}</div><div className="send-row"><Tokens payload={{kind:"visual",template,noun,question}} mode={mode} lang={lang} onWord={showWord}/><Button size="icon" className="send-button" disabled={busy||!!dmWaiting||roomLoading} onClick={()=>send()} aria-label={t("Enviar mensagem","Send message")}><Send/></Button></div></>:<BlockComposer value={blockDraft} onChange={setBlockDraft} lang={lang} busy={busy||!!dmWaiting||roomLoading} onSend={()=>void send()}/>}
             <details className="more-patterns"><summary>{t("Outros moldes e histórias","More patterns and stories")}</summary><div><Button variant="ghost" size="sm" onClick={()=>setComposeMode("blocks")}>{t("10 moldes do método","10 method patterns")}</Button><Button variant="ghost" size="sm" onClick={()=>setComposeMode("visual")}>{t("Frases curtas","Short sentences")}</Button><Button variant="ghost" size="sm" onClick={()=>setComposeMode("story")}>{t("Histórias no passado","Past stories")}</Button></div></details>
            </TabsContent>
            <TabsContent value="text"><form className="text-composer" onSubmit={e=>{e.preventDefault();void send();}}><Input value={text} onChange={e=>setText(e.target.value)} maxLength={1500} aria-label={t("Mensagem","Message")} placeholder={t("Escreva em japonês ou em seu idioma…","Write in Japanese or your language…")}/><Button size="icon" className="send-button" type="submit" disabled={busy||!text.trim()||!!dmWaiting||roomLoading} aria-label={t("Enviar mensagem","Send message")}><Send/></Button></form><p className="composer-note">{t("Texto livre segue como você escreveu. Use os moldes para receber a explicação da estrutura.","Free text is sent as written. Use the patterns to receive a structure explanation.")}</p></TabsContent>
          </Tabs>
