...
import { ArrowLeft, ArrowUpRight, Check, ChevronRight, Globe2, Image, Languages, Leaf, LogOut, MessageCircle, Plus, Send, Settings2, Shield, Users, X } from "lucide-react";
...
import { avatars, compose, lexicon, templates, type Lang, type MessagePayload, type Word } from "@/lib/vocabulary";
...
function Tokens({payload,mode,lang,onWord}:{payload:MessagePayload;mode:Mode;lang:Lang;onWord:(word:Word)=>void}) {
  ...
  return <div className={"tokens mode-"+mode} lang="ja" aria-label={sentence.japanese}>{sentence.tokens.map((id,index)=>{
    const w=lexicon[id];return <button key={id+index} type="button" className={"word-token"+(w.grammar?" grammar-token":"")} onClick={()=>onWord(w)} aria-label={w.jp+" · "+w[lang]}>
...
export default function ChatApp({signedIn,signInPath,signOutPath}:{signedIn:boolean;signInPath:string;signOutPath:string}) {
  ...
  const [composeMode,setComposeMode]=useState("visual");
  ...
  const draft:MessagePayload=composeMode==="text"?{kind:"text",text}:{kind:"visual",template,noun,question};
  const me=data.me;
  ...
  const showWord=(w:Word)=>{setWord(w);setReveal(true);};
  ...
  const demo:MessagePayload={kind:"visual",template:"see",noun:"tree"};
  ...
  return <div className="app-frame">
    ...
    <div className={"chat-shell"+(mobileChat?" mobile-chat":"")}>
      <aside className="people-panel">
        <header className="brand-row"><div className="brand"><span className="brand-mark" lang="ja">木</span><div><strong>kotoba<span className="brand-period">.</span></strong><span>{t("JAPONÊS EM IMAGENS","JAPANESE IN PICTURES")}</span></div></div><Button variant="ghost" size="icon" aria-label={t("Mudar idioma para inglês","Switch language to Portuguese")} onClick={()=>setLang(lang==="pt"?"en":"pt")}><Languages/></Button></header>
      ...
      <main className="conversation-panel">
        ...
        <div className="message-stream" role="log" aria-label={t("Mensagens","Messages")} aria-live="polite">
          {!active&&<div className="practice-intro"><span className="practice-stamp" lang="ja">木</span><h2>{t("Toque na imagem. Descubra a palavra.","Tap a picture. Discover a word.")}</h2><p>{t("Monte uma frase abaixo. Depois, tente ler sem as figuras.","Build a sentence below. Then try reading without the pictures.")}</p><div className="example-bubble"><span className="bubble-label">{t("EXEMPLO","EXAMPLE")}</span>{mode!=="japanese"&&<SentenceScene payload={demo} lang={lang} autoPlay writing={mode==="bridge"}/>}<Tokens payload={demo} mode={mode} lang={lang} onWord={showWord}/><details><summary>{t("Revelar leitura e sentido","Reveal reading and meaning")}</summary><p lang="ja">き を みます。</p><p>ki o mimasu · {t("Vejo uma árvore.","I see a tree.")}</p></details></div></div>}
        ...
        <div className="composer">
          ...
          <Tabs value={composeMode} onValueChange={setComposeMode}><div className="composer-top"><TabsList><TabsTrigger value="visual"><Image/>{t("Com figuras","Pictures")}</TabsTrigger><TabsTrigger value="text"><Languages/>{t("Texto livre","Free text")}</TabsTrigger></TabsList>{!active&&<span className="practice-only">{t("PRÁTICA","PRACTICE")}</span>}</div>
