...
export async function jsonRequest(path:string,body?:unknown,apiOrigin="",sessionToken="") {
 ...
 if(!response.ok){if(response.status===401&&(apiOrigin||sessionToken))window.dispatchEvent(new Event("ojiisan-session-expired"));throw new Error(data.error||"unavailable");}
 return data;
}
