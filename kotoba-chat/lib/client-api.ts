...
export async function jsonRequest(path:string,body?:unknown,apiOrigin="",sessionToken="",signal?:AbortSignal) {
 ...
 const response=await fetch(apiEndpoint(apiOrigin,path),{method:body===undefined?"GET":"POST",headers,body:body===undefined?undefined:JSON.stringify(body),credentials:apiOrigin?"omit":"same-origin",cache:"no-store",signal});
